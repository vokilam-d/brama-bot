      ...
      switch (command) {
        ...
        case AdminBotCommand.DeleteMessages: {
          ...
          for (const messageLink of messageLinks) {
            ...
            const hasLetters = /[a-zA-Z]/.test(pathParts[0]);
            const deletedMessageChatId = hasLetters ? `@${pathParts[0]}` : `-100${parseInt(pathParts[0])}`;
            const deletedMessageId = parseInt(pathParts[2] ?? pathParts[1]);
